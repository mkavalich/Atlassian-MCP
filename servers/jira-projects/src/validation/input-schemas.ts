...
export const createProjectInputSchema = z.object({
  ...
  leadAccountId: z.string().max(255).describe('The account ID of the project lead'),
...
export const getProjectInputSchema = z.object({
  projectIdOrKey: z.string().max(255).describe('The project ID or key'),
  expand: z.string().max(1000).optional().describe('Use expand to include additional information'),
...
export const updateProjectInputSchema = z.object({
  ...
  leadAccountId: z.string().max(255).optional().describe('The new project lead account ID'),
...
export const updateIssueTypeInputSchema = z.object({
  issueTypeId: z.string().max(255).describe('The ID of the issue type to update'),
...
export const deleteIssueTypeInputSchema = z.object({
  issueTypeId: z.string().max(255).describe('The ID of the issue type to delete'),
  alternativeIssueTypeId: z.string().max(255).optional()
...
export const createIssueTypeSchemeInputSchema = z.object({
  ...
  issueTypeIds: z.array(z.string().max(255)).describe('Array of issue type IDs to include in the scheme'),
  defaultIssueTypeId: z.string().max(255).describe('The ID of the default issue type for this scheme'),
}).passthrough();

export const updateIssueTypeSchemeInputSchema = z.object({
  schemeId: z.string().max(255).describe('The ID of the issue type scheme to update'),
...
export const deleteIssueTypeSchemeInputSchema = z.object({
  schemeId: z.string().max(255).describe('The ID of the issue type scheme to delete'),
}).passthrough();
...
export const getIssueTypeSchemeMappingsInputSchema = z.object({
  ...
  projectId: z.array(z.string().max(255)).optional()
    .describe('Filter by project IDs to get their scheme mappings'),
}).passthrough();

export const addIssueTypesToSchemeInputSchema = z.object({
  schemeId: z.string().max(255).describe('The ID of the issue type scheme to modify'),
  issueTypeIds: z.array(z.string().max(255)).min(1)
    .describe('Array of issue type IDs to add to the scheme'),
}).passthrough();

export const assignIssueTypeSchemeToProjectInputSchema = z.object({
  projectId: z.string().max(255).describe('The ID of the project to assign the scheme to'),
  issueTypeSchemeId: z.string().max(255).describe('The ID of the issue type scheme to assign'),
}).passthrough();

// Issue Createmeta input schemas
export const getIssueCreatemetaFieldsInputSchema = z.object({
  projectIdOrKey: z.string().min(1).max(255).describe('The project ID or key (e.g., "PROJ" or "10001")'),
  issueTypeId: z.string().min(1).max(255).describe('The issue type ID (use get_issue_createmeta_issuetypes to find valid IDs)'),
...
export const getIssueEditmetaFieldsInputSchema = z.object({
  issueIdOrKey: z.string().min(1).max(255).describe('The issue ID or key (e.g., "PROJ-123")'),
}).passthrough();
...
export const getDashboardInputSchema = z.object({
  dashboardId: z.string().max(255).describe('The ID of the dashboard'),
}).passthrough();

export const updateDashboardInputSchema = z.object({
  dashboardId: z.string().max(255).describe('The ID of the dashboard to update'),
...
export const deleteDashboardInputSchema = z.object({
  dashboardId: z.string().max(255).describe('The ID of the dashboard to delete'),
}).passthrough();
...
export const generateProjectReportInputSchema = z.object({
  projectKey: z.string().max(255).describe('The project key to generate report for'),
  ...
  dateRange: z.string().max(32).optional().describe('Date range for report (e.g., "30d", "1w")'),
}).passthrough();

// Issue input schemas
export const createIssueInputSchema = z.object({
  projectKey: z.string().min(1).max(10).describe('The project key where the issue will be created (e.g., "PROJ")'),
  issueType: z.string().min(1).max(255).describe('The issue type name (e.g., "Bug", "Task", "Story", "Epic")'),
  summary: z.string().min(1).max(255).describe('The issue summary/title - a brief description of the issue'),
  description: z.string().max(32768).optional().describe('Detailed description of the issue. Supports plain text or Atlassian Document Format (ADF)'),
  assignee: z.string().max(255).optional().describe('The account ID of the user to assign the issue to. Use search_site_users to find account IDs'),
  reporter: z.string().max(255).optional().describe('The account ID of the reporter. Defaults to the authenticated user'),
  priority: z.string().max(255).optional().describe('Priority name (e.g., "Highest", "High", "Medium", "Low", "Lowest")'),
  labels: z.array(z.string().max(255)).optional().describe('Array of label names to apply to the issue'),
  components: z.array(z.string().max(255)).optional().describe('Array of component names or IDs to associate with the issue'),
  ...
  parentKey: z.string().max(255).optional().describe('Parent issue key (required for subtasks, e.g., "PROJ-100")'),
  dueDate: z.string().max(64).optional().describe('Due date in YYYY-MM-DD format (e.g., "2024-12-31")'),
...
export const getIssueInputSchema = z.object({
  issueIdOrKey: z.string().min(1).max(255).describe('The issue ID or key (e.g., "PROJ-123" or "10001")'),
  fields: z.array(z.string().max(255)).optional().describe('Specific fields to return (e.g., ["summary", "status", "assignee"]). Omit for all fields'),
...
export const updateIssueInputSchema = z.object({
  issueIdOrKey: z.string().min(1).max(255).describe('The issue ID or key to update (e.g., "PROJ-123")'),
  ...
  assignee: z.string().max(255).nullable().optional().describe('New assignee account ID. Set to null to unassign'),
  priority: z.string().max(255).optional().describe('New priority name (e.g., "High", "Medium", "Low")'),
  labels: z.array(z.string().max(255)).optional().describe('New array of labels (replaces all existing labels)'),
  components: z.array(z.string().max(255)).optional().describe('New array of component names or IDs'),
  fixVersions: z.array(z.string().max(255)).optional().describe('New array of fix version names or IDs'),
  dueDate: z.string().max(64).nullable().optional().describe('New due date (YYYY-MM-DD format) or null to clear'),
...
export const deleteIssueInputSchema = z.object({
  issueIdOrKey: z.string().min(1).max(255).describe('The issue ID or key to delete (e.g., "PROJ-123")'),
...
const bulkIssueItemSchema = z.object({
  ...
  issueType: z.string().min(1).max(255).describe('The issue type name (e.g., "Bug", "Task", "Story")'),
  ...
  assignee: z.string().max(255).optional().describe('Account ID of the assignee'),
  priority: z.string().max(255).optional().describe('Priority name (e.g., "High", "Medium", "Low")'),
...
export const getTransitionsInputSchema = z.object({
  issueIdOrKey: z.string().min(1).max(255).describe('The issue ID or key (e.g., "PROJ-123")'),
  expand: z.string().max(1000).optional().describe('Use "transitions.fields" to include fields required for each transition'),
}).passthrough();

export const transitionIssueInputSchema = z.object({
  issueIdOrKey: z.string().min(1).max(255).describe('The issue ID or key to transition (e.g., "PROJ-123")'),
  transitionId: z.string().max(255).optional().describe('The transition ID to perform. Use get_transitions to find available IDs'),
  transitionName: z.string().max(255).optional().describe('The transition name (e.g., "Start Progress", "Done"). Alternative to transitionId'),
  comment: z.string().max(32768).optional().describe('Optional comment to add during the transition'),
  resolution: z.string().max(255).optional().describe('Resolution name (e.g., "Done", "Won\'t Do") - required for some transitions'),
...
export const assignIssueInputSchema = z.object({
  issueIdOrKey: z.string().min(1).max(255).describe('The issue ID or key to assign (e.g., "PROJ-123")'),
  accountId: z.string().max(255).nullable().describe('Account ID of the user to assign. Set to null to unassign the issue. Use search_site_users to find account IDs'),
}).passthrough();

// Comment input schemas
export const addCommentInputSchema = z.object({
  issueIdOrKey: z.string().min(1).max(255).describe('The issue ID or key to add a comment to (e.g., "PROJ-123")'),
  body: z.string().min(1).max(32768).describe('The comment text. Supports plain text or Atlassian Document Format (ADF)'),
  visibility: z.object({
    type: z.enum(['role', 'group']).describe('Restriction type: "role" for project roles, "group" for user groups'),
    value: z.string().max(255).describe('The role name (e.g., "Developers") or group name to restrict visibility to'),
...
export const getCommentsInputSchema = z.object({
  ...
  expand: z.string().max(1000).optional().describe('Use "renderedBody" to include HTML-rendered comment body'),
}).passthrough();

export const updateCommentInputSchema = z.object({
  issueIdOrKey: z.string().min(1).max(255).describe('The issue ID or key (e.g., "PROJ-123")'),
  commentId: z.string().min(1).max(255).describe('The comment ID to update. Use get_comments to find comment IDs'),
  body: z.string().min(1).max(32768).describe('The new comment text'),
  visibility: z.object({
    type: z.enum(['role', 'group']).describe('Restriction type: "role" for project roles, "group" for user groups'),
    value: z.string().max(255).describe('The role name or group name to restrict visibility to'),
  }).optional().describe('Restrict to specific role/group (standard Jira). For Service Desk internal comments, use "internal" instead'),
...
export const deleteCommentInputSchema = z.object({
  issueIdOrKey: z.string().min(1).max(255).describe('The issue ID or key (e.g., "PROJ-123")'),
  commentId: z.string().min(1).max(255).describe('The comment ID to delete. Use get_comments to find comment IDs'),
}).passthrough();

// Attachment input schemas
export const addAttachmentInputSchema = z.object({
  issueIdOrKey: z.string().min(1).max(255).describe('The issue ID or key to attach the file to (e.g., "PROJ-123")'),
  filename: z.string().min(1).max(255).describe('The filename for the attachment (e.g., "screenshot.png", "document.pdf")'),
  content: z.string().min(1).max(50000000).describe('The file content as a base64 encoded string. Encode your file to base64 before sending'),
  mimeType: z.string().max(255).optional().describe('The MIME type of the file (e.g., "application/pdf", "image/png", "text/plain"). If omitted, defaults to "application/octet-stream"'),
}).passthrough();

export const getAttachmentInputSchema = z.object({
  attachmentId: z.string().min(1).max(255).describe('The attachment ID. Use list_issue_attachments to find attachment IDs'),
}).passthrough();

export const listIssueAttachmentsInputSchema = z.object({
  issueIdOrKey: z.string().min(1).max(255).describe('The issue ID or key to list attachments for (e.g., "PROJ-123")'),
}).passthrough();

export const deleteAttachmentInputSchema = z.object({
  attachmentId: z.string().min(1).max(255).describe('The attachment ID to delete. Use list_issue_attachments to find attachment IDs. This action cannot be undone'),
}).passthrough();
...
export const getBoardsInputSchema = z.object({
  ...
  name: z.string().max(255).optional().describe('Filter boards by name (partial match). E.g., "Backend" matches "Backend Team Board"'),
  projectKeyOrId: z.string().max(255).optional().describe('Filter by project key or ID (e.g., "PROJ" or "10001")'),
}).passthrough();
...
export const getBoardBacklogInputSchema = z.object({
  ...
  jql: z.string().max(10000).optional().describe('Additional JQL filter to narrow backlog results (e.g., "priority = High")'),
  fields: z.array(z.string().max(255)).optional().describe('Specific issue fields to return (e.g., ["summary", "status", "priority"])'),
}).passthrough();
...
export const createBoardInputSchema = z.object({
  ...
  projectKeyOrId: z.string().max(255).optional().describe('Project to associate the board with (e.g., "PROJ" or "10001"). If omitted, board is user-scoped'),
}).passthrough();
...
export const createSprintInputSchema = z.object({
  ...
  goal: z.string().max(32768).optional().describe('Sprint goal - what the team aims to achieve this sprint'),
  startDate: z.string().max(64).optional().describe('Start date in ISO 8601 format (e.g., "2024-01-15T09:00:00.000Z"). Leave empty for unscheduled sprint'),
  endDate: z.string().max(64).optional().describe('End date in ISO 8601 format (e.g., "2024-01-29T17:00:00.000Z"). Typically 1-4 weeks after start'),
}).passthrough();
...
export const updateSprintInputSchema = z.object({
  ...
  completeDate: z.string().max(64).optional().describe('Complete date in ISO 8601 format. Only set when closing a sprint (state="closed")'),
}).passthrough();
...
export const getSprintIssuesInputSchema = z.object({
  ...
  startAt: z.number().min(0).optional().default(0).describe('Starting index for pagination'),
  maxResults: z.number().min(1).max(100).optional().default(50).describe('Maximum issues to return (1-100)'),
  jql: z.string().max(10000).optional().describe('Additional JQL filter (e.g., "status = Done" to see completed issues)'),
  fields: z.array(z.string().max(255)).optional().describe('Specific issue fields to return (e.g., ["summary", "status", "assignee"])'),
}).passthrough();
...
export const moveIssuesToSprintInputSchema = z.object({
  ...
  issues: z.array(z.string().max(255)).min(1).max(50).describe('Issue keys to move (e.g., ["PROJ-1", "PROJ-2"]). Maximum 50 issues per request'),
  rankBefore: z.string().max(255).optional().describe('Issue key to rank the moved issues before (higher priority)'),
  rankAfter: z.string().max(255).optional().describe('Issue key to rank the moved issues after (lower priority)'),
}).passthrough();

export const moveIssuesToBacklogInputSchema = z.object({
  issues: z.array(z.string().max(255)).min(1).max(50).describe('Issue keys to move to backlog (e.g., ["PROJ-1", "PROJ-2"]). Removes from any sprint. Maximum 50 issues'),
}).passthrough();
