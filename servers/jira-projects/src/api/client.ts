...
export class JiraApiClient {
  ...
  private handleError(error: AxiosError): never {
    ...
    if (status) {
      ...
      // Add request context for debugging (re-sanitize since this runs after the constructor)
  ...
  async makeRequest<T>(config: RequestConfig): Promise<ApiResponse<T>> {
    ...
    try {
      ...
      // used to sit here was constant-false over every registered tool path, but it pre-wired the
      // wrong credential for whoever next added an /organization- or /instance-prefixed path.
      const authHeaders = this.authManager.getAuthHeaders();
      
      const apiBase = config.apiBase || '/rest/api/3';
      const baseURL = `${this.authManager.getBaseUrl()}${apiBase}`;
